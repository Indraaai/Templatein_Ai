                ...
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        <h4 class="text-sm font-semibold text-blue-800">
                            Informasi Tambah Mahasiswa
                        </h4>
                        <p class="text-sm text-blue-700 mt-1">
                            Pastikan data mahasiswa yang diinput sudah benar. Email akan digunakan sebagai username
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                        ...
                        .then(data => {
                            ...
                            if (data && Array.isArray(data) && data.length > 0) {
                                ...
                                data.forEach((prodi, index) => {
                                    ...
                                    // Pre-select if this was the selected value
                                    if (selectedProdiId && prodi.id == selectedProdiId) {
                ...
                // Load program studies on page load if faculty is already selected
                @if (old('faculty_id'))
                    const oldFacultyId = '{{ old('faculty_id') }}';
                    const oldProdiId = '{{ old('program_study_id') }}';
                    console.log('Loading old values - Faculty:', oldFacultyId, 'Prodi:', oldProdiId);
                    if (oldFacultyId) {
                        loadProgramStudies(oldFacultyId, oldProdiId);
                    }
                @endif
